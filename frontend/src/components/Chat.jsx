...
const Chat = () => {
  ...
  const sendMessage = () => {
    if (!message.trim() || !user || !selectedUser) return;
    const chatMessage = { sender: user._id, content: message, chatRoom: selectedUser._id };
    socket.emit("sendMessage", chatMessage);
    setMessages((prev) => [...prev, { sender: { _id: user._id, username: user.username }, content: message, timestamp: new Date().toISOString() }]);
    setMessage("");
  };
    ...
    <div className="chat-app mt-5">
      <div className="sidebar mt-5">
        <div className="sidebar-header">
          ...
          <Button variant="link" onClick={logout}><FiLogOut size={20} /></Button>
        </div>
        <div className="search-box">
          <Form.Control type="text" placeholder="Search users..." className="search-input" />
        </div>
        <div className="users-list">
          {loadingUsers ? <Spinner animation="border" /> : users.map((u) => (
        ...
        {selectedUser ? (
          <>
            ...
            <div className="messages-container">
              {loadingMessages ? <Spinner animation="border" /> : messages.map((msg, index) => (
            ...
            <div className="message-input">
              <Form.Control as="textarea" rows={1} value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} className="message-textarea" />
